...
function webgl_run()
{
	...
	for (x = 0; x<_world.map_size - 1; x++)
	{
		...
		{
			_world.map[x][y][0] += Math.random() * 0.5 - 0.25;
			_world.map[x][y][1] += Math.random() * 0.5 - 0.25;
		}
	}
			...
			switch (_world.map[x][y][3])
			{
				case 1: // water/coast
					if (_world.map[x][y][2] > _sea_level - _coast_x)
					{
						...
						d = rgb_interpolate_array(d[0], d[1], d[2], 215/255, 215/255, 100/255, clamp(normalize(_world.map[x][y][2], _sea_level, _sea_level - _coast_x) * 2, 0, 1));
					}
					else
					{
						// water
						d = rgb_interpolate_array(10/255, 60/255, 180/255, 192/255, 192/255, 160/255, f * clamp(normalize(_world.map[x][y][2], (_sea_level - _coast_x) * 0.66, _sea_level - _coast_x) * 0.7, 0, 1));
					}
				break;
